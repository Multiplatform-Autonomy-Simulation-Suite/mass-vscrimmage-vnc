...
echo -e "\n------------------ start noVNC  ----------------------------"
echo -e "\n-------------parsing noVNC Launch Arguments-----------------"
if [[ $VNC_CERT != "" ]]; then SECURE_ARGS="--cert ${VNC_CERT} --ssl-only"; fi
if [[ $DEBUG == true ]]; then echo "$NO_VNC_HOME/utils/launch.sh ${SECURE_ARGS} --vnc localhost:$VNC_PORT --listen $NO_VNC_PORT"; fi
# shellcheck disable=SC2086
"$NO_VNC_HOME"/utils/launch.sh $SECURE_ARGS --vnc localhost:$VNC_PORT --listen $NO_VNC_PORT &> "$STARTUPDIR"/no_vnc_startup.log &
PID_SUB=$!
